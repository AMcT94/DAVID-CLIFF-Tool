 ...
 function.tanfBenefit<-function(data){
  ...
  temp<-temp %>% 
   ...
   if(state_code %in% unique(data$stateFIPS)){ # make sure that state is in the list
     ...
     # Step I: Calculate net income
     temp$net.income<-rowMaxs(cbind(temp$EarnedIncomeDisregard*(temp$StandardOfNeed - (temp$income/12)),0)) # earned income deduction
     temp$net.income <- temp$net.income + (temp$value.ssdi/12)
   ...
   if(state_code %in% unique(data$stateFIPS)){ # make sure that state is in the list
     ...
     temp <- temp %>%
       ...
       group_by(householdid) %>% 
       mutate(min_Year=min(Year)) %>% 
       mutate(net.income=ifelse(Year==(min_Year), net.income-450, net.income)) %>%
